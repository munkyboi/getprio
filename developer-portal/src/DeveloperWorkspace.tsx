...
function Setup({ allowance }: { allowance: SandboxAllowance | null }) {
  ...
  return <section className="developer-workspace-panel dpp-section"><h1>Your first ticket, end to end.</h1><p>Install the tester app, sign in with a test account, then follow a ticket through your integration.</p><Allowance value={allowance} /><div className="developer-workspace-setup-grid dpp-steps">
    ...
    <Paper component="article" withBorder className="developer-workspace-setup-card"><p className="developer-workspace-eyebrow">03 / VERIFY</p><h3>Follow a test ticket</h3><ol><li>Use your sandbox key from your backend.</li><li>Create a queue and issue one test ticket.</li><li>Scan its private link, or accept its in-app invitation.</li><li>Call the ticket and verify the app update and push.</li></ol><p>A public queue QR does not claim an existing ticket.</p><Button component="a" href="/docs" variant="light">Open quickstart</Button></Paper>
  </div>{notice && <p className="developer-workspace-setup-action-notice" role="status">{notice}</p>}</section>;
}

function ApiKeyCreatePage({ project, busy, onCreate, onBack, onDirtyChange }: { project: Project; busy: string; onCreate: (event: FormEvent<HTMLFormElement>) => void; onBack: () => void; onDirtyChange: (dirty: boolean) => void }) {
  const [selectedScopes, setSelectedScopes] = useState<string[]>([]);
  useEffect(() => { onDirtyChange(selectedScopes.length > 0); }, [onDirtyChange, selectedScopes.length]);
  return <section className="developer-workspace-panel dpp-section developer-workspace-key-create-page">
    ...
    <Paper component="form" withBorder p="xl" onSubmit={onCreate} className="developer-workspace-key-create">
      ...
      <Select label="Profile access" data={[{ value: "all", label: "All profiles" }]} value="all" readOnly comboboxProps={{ withinPortal: false }} classNames={{ dropdown: "dpp-select-dropdown", option: "dpp-select-option" }} />
      <p className="developer-workspace-key-note">Includes profiles added later. Creating profiles also requires profiles:write.</p>
      ...
      <div className="dpp-key-create-action"><Button type="submit" className="developer-workspace-primary" loading={busy === "key"} disabled={!selectedScopes.length || busy === "key"} leftSection={<IconKey size={16} />}>Generate Sandbox key</Button></div>
      <p>Base URL</p>
...
function ProfilesPage({ profiles, onCreatePage, busy, onUpdateProfile }: { profiles: Profile[]; onCreatePage: () => void; busy: string; onUpdateProfile: (profile: Profile, event: FormEvent<HTMLFormElement>) => Promise<boolean> }) {
    ...
    <div className="developer-workspace-resource-grid developer-workspace-profile-grid">
      <Paper withBorder p="xl" className="developer-workspace-resource-card developer-workspace-resource-summary">
        ...
        {profiles.length ? <div className="developer-workspace-profile-list">{profiles.map((profile) => {
          ...
          </Paper> : <div className="dpp-row" key={profile.id}><div><strong>{profile.displayName}</strong><p><code>{profile.slug}</code></p><small>Private Sandbox profile · Directory {directoryStatusLabel(profile.directoryStatus).toLowerCase()}</small></div><div className="developer-workspace-profile-row-actions"><Badge color={profile.directoryStatus === "approved" ? "green" : "blue"} variant="light" radius="xl">{directoryStatusLabel(profile.directoryStatus)}</Badge><Button type="button" variant="subtle" size="compact-sm" onClick={() => setEditingProfileId(profile.id)} leftSection={<IconPencil size={15} />}>Edit</Button></div></div>;
...
function QueuesPage({ profiles, selectedProfile, selectedProfileValue, queues, snapshots = [], busy, onProfileChange, onCreatePage, onUpdateQueue }: { profiles: Profile[]; selectedProfile: string; selectedProfileValue: Profile | null; queues: Queue[]; snapshots?: QueueSnapshot[]; busy: string; onProfileChange: (value: string) => void; onCreatePage: () => void; onUpdateQueue: (queue: Queue, event: FormEvent<HTMLFormElement>) => Promise<boolean> }) {
  ...
  const liveTickets = selectedSnapshot ? [selectedSnapshot.current, ...selectedSnapshot.nextUp, ...selectedSnapshot.overflow].filter((ticket): ticket is DeveloperTicket => Boolean(ticket)) : [];
...
function WebhooksPage({ webhooks, selectedWebhook, selectedWebhookValue, deliveries, busy, onWebhookChange, onCreate, onRotate, onCompromised, onDisable, onRefresh, onReplay }: { webhooks: Webhook[]; selectedWebhook: string; selectedWebhookValue: Webhook | null; deliveries: Delivery[]; busy: string; onWebhookChange: (value: string) => void; onCreate: (event: FormEvent<HTMLFormElement>) => void; onRotate: (webhook: Webhook) => void; onCompromised: (webhook: Webhook) => void; onDisable: (webhook: Webhook) => void; onRefresh: () => void; onReplay: (delivery: Delivery) => void }) {
  ...
  return <section className="developer-workspace-panel dpp-section developer-workspace-webhooks-page">
    ...
    <Paper component="form" withBorder p="xl" onSubmit={(event) => { onCreate(event); setSelectedEvents([]); }} className="developer-workspace-resource-card developer-workspace-webhook-form">
      ...
      <div className="dpp-application-fields"><TextInput label="Destination URL" name="url" type="url" required placeholder="https://example.com/getprio/webhooks" /><Select label="Payload version" data={[{ value: "1", label: "v1" }]} value="1" readOnly comboboxProps={{ withinPortal: false }} classNames={{ dropdown: "dpp-select-dropdown", option: "dpp-select-option" }} /></div>
      <TextInput label="Receiver name" name="name" required maxLength={80} placeholder="Production event listener" />
...
export default function DeveloperWorkspace({ session, accountContent, light = false }: Props) {
  ...
  function navigate(nextSection: Section, nextEnvironment: Environment = environment) {
    if (keyCreateDirty && section === "keyCreate" && nextSection !== "keyCreate") {
  ...
  useEffect(() => {
    const syncLocation = () => {
      ...
      if (keyCreateDirty && section === "keyCreate" && next.section !== "keyCreate") {
  ...
  }, [environment, keyCreateDirty, section]);
  useEffect(() => {
    if (!keyCreateDirty) return;
  ...
  async function createKey(event: FormEvent<HTMLFormElement>) {
    ...
    await submit("key", async () => { const result = await developerApi.createKey(projectId, { name: String(form.get("name") || `${project?.name || "Sandbox"} Sandbox key`), environment: "sandbox", scopes: selectedScopes }, session.csrfToken); setKeyCreateDirty(false); setSecret({ label: "Sandbox API key", value: result.secret, warning: result.warning }); await refreshWorkspace(); formElement.reset(); navigate("keys", "sandbox"); });
  }
  ...
  async function createWebhook(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const formElement = event.currentTarget; const form = new FormData(formElement); const events = webhookEvents.filter((name) => form.get(name) === "on");
    await submit("webhook", async () => { const result = await developerApi.createWebhook(projectId, { name: String(form.get("name") || ""), url: String(form.get("url") || ""), events }, session.csrfToken); setSecret({ label: "Webhook signing secret", value: result.secret, warning: result.warning }); await refreshWorkspace(); formElement.reset(); });
  }

  const isAccountPage = accountContent !== undefined;
  ...
  return <section className="developer-workspace dpp dpp-A" data-api-theme={light ? "light" : "dark"}>
    ...
    <Modal opened={Boolean(pendingNavigation)} onClose={() => setPendingNavigation(null)} closeButtonProps={{ "aria-label": "Close discard confirmation", size: 44 }} title="Discard unsaved API key?" centered size="sm" withinPortal={false} classNames={{ content: "developer-workspace-discard-modal", header: "developer-workspace-discard-modal-header", body: "developer-workspace-discard-modal-body" }}>
      <p>Your selected API-key permissions and profiles will be discarded. Existing keys will remain intact.</p>
    ...
    <aside className="developer-workspace-sidebar dpp-nav" aria-label="Developer workspace navigation">
      ...
      <Select label="Active project" placeholder="Select a project" value={projectId || null} onChange={(value) => { const nextProjectId = value || ""; if (!nextProjectId || nextProjectId === projectId) return; if (keyCreateDirty && section === "keyCreate") { setPendingNavigation({ section: "overview", environment: "sandbox", projectId: nextProjectId }); return; } setProjectId(nextProjectId); }} data={projects.map((item) => ({ value: item.id, label: item.name }))} comboboxProps={{ withinPortal: false }} classNames={{ dropdown: "dpp-select-dropdown", option: "dpp-select-option" }} />
    ...
    <div className="developer-workspace-context dpp-context"><span>{isAccountPage ? "Account / Profile" : ["billing", "subscriptions", "teamSecurity", "security"].includes(section) ? "Account" : project?.name || "Developer account"} {!isAccountPage && ` / ${sectionLabels[section]}`}</span>{!isAccountPage && !["billing", "subscriptions", "teamSecurity", "security"].includes(section) && <Badge className={`developer-workspace-status ${environment}`} color="blue" variant="filled" radius="xl">{environment}</Badge>}</div>
    ...
    {loading ? <p className="developer-workspace-loading">Loading your workspace…</p> : isAccountPage ? accountContent : <>
      ...
      {section === "security" && <SecurityPage session={session} />}
      {["billing", "subscriptions", "teamSecurity", "security"].includes(section) ? null : !project ? <section className="developer-workspace-panel dpp-section"><p className="developer-workspace-eyebrow">FIRST STEP</p><h1>Create your Sandbox project</h1><p>A project owns its keys, profiles, queues, and webhook registrations. The current plan supports one active project.</p><form onSubmit={(event) => void createProject(event)} className="developer-workspace-form"><TextInput label="Project name" name="name" required maxLength={80} placeholder="Harbor Services integration" /><Button type="submit" className="developer-workspace-primary" loading={busy === "project"} leftSection={<IconPlus size={16} />}>Create project</Button></form></section> : <>
        ...
        {section === "profiles" && <ProfilesPage profiles={profiles} busy={busy} onCreatePage={() => navigate("profileCreate", "sandbox")} onUpdateProfile={updateProfile} />}
