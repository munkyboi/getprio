...
function Setup({ allowance, testAccounts, busy, onCreate, onCreateAppleReview, onReset }: { allowance: SandboxAllowance | null; testAccounts: SandboxTestAccount[]; busy: string; onCreate: () => void; onCreateAppleReview: () => void; onReset: (account: SandboxTestAccount) => void }) {
  ...
  return <section className="developer-workspace-panel dpp-section"><h1>Your first ticket, end to end.</h1><p>Install the tester app, sign in with a test account, then follow a ticket through your integration.</p><Allowance value={allowance} /><div className="developer-workspace-setup-grid dpp-steps">
    ...
    <Paper component="article" withBorder className="developer-workspace-setup-card"><p className="developer-workspace-eyebrow">02 / SIGN IN</p><h3>Test accounts · {accountCount} / 2</h3><p>Developer credentials expire after seven days. The dedicated Apple review account expires after 30 days. Resetting credentials ends previous sessions and removes registered devices. Tickets addressed to the account email appear in the Sandbox app.</p>{testAccounts.length > 0 && <div className="developer-workspace-test-account-list">{testAccounts.map((account) => <div className="developer-workspace-test-account" key={account.id}><div><strong>{account.purpose === "apple_review" ? "Apple review account" : account.username}</strong><small>{account.status === "active" ? `Expires ${formatDate(account.expiresAt)}` : "Expired · reset to reuse"} · {account.deviceCount} device{account.deviceCount === 1 ? "" : "s"}</small></div><Button type="button" variant="light" loading={busy === `test-account-reset-${account.id}`} disabled={provisioningBusy} onClick={() => onReset(account)}>{account.status === "active" ? "Reset" : "Reset & reuse"}</Button></div>)}</div>}<div className="developer-workspace-inline-actions"><Button type="button" loading={busy === "test-account-create"} disabled={provisioningBusy || accountCount >= 2} onClick={onCreate}>{accountCount >= 2 ? "Two accounts created" : "Create test account"}</Button><Button type="button" variant="light" loading={busy === "test-account-review-create"} disabled={provisioningBusy || accountCount >= 2 || testAccounts.some((account) => account.purpose === "apple_review")} onClick={onCreateAppleReview}>{testAccounts.some((account) => account.purpose === "apple_review") ? "Apple review account created" : "Create Apple review account"}</Button></div></Paper>
  ...
  return <section className="developer-workspace-panel dpp-section developer-workspace-queue-create-page">
    ...
    {!profiles.length ? <Paper withBorder p="xl" className="developer-workspace-resource-card"><h2>Create a profile first</h2><p>Queues belong to a private profile. Create one before adding a queue.</p></Paper> : <>
      <Paper component="form" withBorder p="xl" onSubmit={onCreateQueue} className="developer-workspace-resource-card developer-workspace-queue-form">
        <div className="dpp-application-fields"><TextInput label="Queue name" name="displayName" required maxLength={120} placeholder="Main service desk" /><TextInput label="Queue slug" name="slug" required maxLength={64} pattern="[a-z0-9]+(?:-[a-z0-9]+)*" placeholder="main-service-desk" /><TextInput label="Queue prefix" name="queuePrefix" maxLength={4} pattern="[A-Za-z0-9]{1,4}" placeholder="MAIN" description="Defaults to the first 4 characters of the queue slug." /><TextInput label="Average service minutes" name="averageServiceMinutes" type="number" min={1} max={120} defaultValue={15} required description="Used for customer wait estimates." /><TextInput label="Notify when within" name="notificationThreshold" type="number" min={1} max={10} defaultValue={2} required description="Waiting position that triggers an almost-next notification." /><Select label="Initial state" name="sessionState" defaultValue="closed" data={[{ value: "closed", label: "Closed" }, { value: "open", label: "Open" }, { value: "paused", label: "Paused" }]} comboboxProps={{ withinPortal: false }} classNames={{ dropdown: "dpp-select-dropdown", option: "dpp-select-option" }} /><Switch className="developer-workspace-intake-switch" name="intakeEnabled" label="Allow ticket intake" /></div>
...
function QueuesPage({ profiles, selectedProfile, selectedProfileValue, queues, snapshots = [], busy, onProfileChange, onCreatePage, onUpdateQueue, onDeleteQueue }: { profiles: Profile[]; selectedProfile: string; selectedProfileValue: Profile | null; queues: Queue[]; snapshots?: QueueSnapshot[]; busy: string; onProfileChange: (value: string) => void; onCreatePage: () => void; onUpdateQueue: (queue: Queue, event: FormEvent<HTMLFormElement>) => Promise<boolean>; onDeleteQueue: (queue: Queue) => void }) {
  ...
  const ticketRows = liveTickets.map((ticket, index, all) => ({
    ticket,
  ...
  const waitingCount = selectedSnapshot?.stats.waitingCount || 0;
  const calledCount = selectedSnapshot?.stats.calledCount || 0;
  ...
  return <section className="developer-workspace-panel dpp-section developer-workspace-queues-page">
    ...
    {!profiles.length ? <Paper withBorder p="xl" className="developer-workspace-resource-card"><h2>No queues in this environment.</h2><p>Create a profile first, then add its queue here.</p></Paper> : <>
      ...
      <section className="developer-workspace-resource-section"><div className="developer-workspace-resource-header"><div><p className="developer-workspace-eyebrow">CURRENT QUEUES</p><h2>{selectedProfileValue?.displayName || "Profile"}</h2><p>Queue state, intake, ticket numbering, wait estimates, and near-turn notifications.</p></div>{selectedProfileValue && <Button type="button" className="developer-workspace-primary" onClick={onCreatePage} leftSection={<IconPlus size={16} />}>Create queue</Button>}</div><div className="developer-workspace-queue-list">{queues.length ? queues.map((queue) => { const editing = editingQueueId === queue.id; return <Paper component="article" withBorder p="xl" key={queue.id}>{editing ? <form className="developer-workspace-queue-edit" onSubmit={async (event) => { event.preventDefault(); if (await onUpdateQueue(queue, event)) setEditingQueueId(null); }}><p className="developer-workspace-eyebrow">EDIT QUEUE</p><TextInput label="Queue name" name="displayName" defaultValue={queue.displayName} required maxLength={120} /><TextInput label="Queue slug" value={queue.slug} readOnly description="Queue slugs are permanent identifiers." /><TextInput label="Queue prefix" name="queuePrefix" defaultValue={queue.queuePrefix} maxLength={4} pattern="[A-Za-z0-9]{1,4}" required /><TextInput label="Average service minutes" name="averageServiceMinutes" type="number" min={1} max={120} defaultValue={queue.averageServiceMinutes} required /><TextInput label="Notify when within" name="notificationThreshold" type="number" min={1} max={10} defaultValue={queue.notificationThreshold} required /><Select label="State" name="sessionState" defaultValue={queue.sessionState} data={[{ value: "closed", label: "Closed" }, { value: "open", label: "Open" }, { value: "paused", label: "Paused" }, { value: "closing", label: "Closing" }]} comboboxProps={{ withinPortal: false }} classNames={{ dropdown: "dpp-select-dropdown", option: "dpp-select-option" }} /><Switch className="developer-workspace-intake-switch" name="intakeEnabled" label="Allow ticket intake" defaultChecked={queue.intakeEnabled} /><div className="developer-workspace-queue-actions"><Button type="button" variant="subtle" onClick={() => setEditingQueueId(null)}>Cancel</Button><Button type="submit" loading={busy === `queue-update-${queue.id}`} leftSection={<IconCheck size={16} />}>Save Changes</Button></div></form> : <><div><h3>{queue.displayName}</h3><p><code>{selectedProfileValue?.slug}/{queue.slug}</code></p><small>Intake {queue.intakeEnabled ? "enabled" : "disabled"} · prefix {queue.queuePrefix} · {queue.averageServiceMinutes} min average · notify within {queue.notificationThreshold}</small></div><div className="developer-workspace-queue-actions"><Badge color={queue.sessionState === "open" ? "teal" : "gray"} variant="light" radius="xl">{queue.sessionState}</Badge><Button type="button" variant="light" onClick={() => setEditingQueueId(queue.id)}>Edit queue</Button><Button type="button" variant="subtle" color="red" onClick={() => onDeleteQueue(queue)}>Delete</Button></div></>}</Paper>; }) : <Empty title="No queues in this profile">Create a queue before issuing tickets with the API.</Empty>}</div></section>
      {selectedQueue && <section className="developer-workspace-resource-section developer-workspace-ticket-section">
        ...
        {selectedSnapshot ? <>
          ...
          <p>Mobile tracking is not exposed by the Developer Portal API yet. On narrow screens, scroll horizontally to view every column.</p>
          <div className="dpp-delivery-table-wrap" role="region" aria-label="Current queue tickets" tabIndex={0}><table className="dpp-delivery-table developer-workspace-ticket-table"><caption>{selectedQueue.displayName} · Sandbox · current tickets</caption><thead><tr><th scope="col"><span className="dpp-sr-only">Expand ticket</span></th><th scope="col">Ticket</th><th scope="col">Status</th><th scope="col">Ahead</th><th scope="col">Mobile tracking</th><th scope="col">Queue</th></tr></thead><tbody>{!filteredTickets.length && <tr><td colSpan={6}><h3>{ticketRows.length ? "No matching tickets" : "No tickets yet"}</h3><p>{ticketRows.length ? "Try another search or clear the filters." : "Tickets issued through your integration will appear here."}</p></td></tr>}{pageRows.map((row) => { const open = expandedTickets.includes(row.ticket.id); const toggle = () => setExpandedTickets(open ? expandedTickets.filter((id) => id !== row.ticket.id) : [...expandedTickets, row.ticket.id]); return <Fragment key={row.ticket.id}><tr className="dpp-delivery-parent" tabIndex={0} aria-expanded={open} aria-controls={`ticket-panel-${row.ticket.id}`} aria-label={`${row.label}, ${row.ticket.status}, ${row.tracking}`} onClick={toggle} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); toggle(); } }}><td><span className={`dpp-row-chevron${open ? " is-open" : ""}`} aria-hidden="true"><svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="m9 5 7 7-7 7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg></span></td><th scope="row">{row.label}</th><td><Badge color={row.ticket.status === "called" ? "teal" : "blue"} variant="light" radius="xl">{row.ticket.status}</Badge></td><td>{row.ahead}</td><td>{row.tracking}</td><td>{selectedQueue.displayName}</td></tr>{open && <tr className="dpp-delivery-note"><td colSpan={6}><div className="dpp-webhook-detail" id={`ticket-panel-${row.ticket.id}`}><section><small>TICKET DETAILS / SANDBOX</small><h3>{row.label}</h3><dl><div><dt>Ticket ID</dt><dd><code>{row.ticket.id}</code></dd></div><div><dt>Display label</dt><dd>{row.ticket.displayLabel || row.label}</dd></div><div><dt>Created at</dt><dd>{formatDate(row.ticket.createdAt)}</dd></div><div><dt>Updated at</dt><dd>{formatDate(row.ticket.updatedAt)}</dd></div><div><dt>Status</dt><dd>{row.ticket.status}</dd></div><div><dt>External reference</dt><dd>{row.ticket.externalReference || "None"}</dd></div><div><dt>Queue ID</dt><dd><code>{row.ticket.queueId}</code></dd></div><div><dt>Waiting tickets ahead</dt><dd>{row.ahead}</dd></div><div><dt>Profile</dt><dd>{selectedProfileValue?.displayName || "Private profile"}</dd></div></dl><p>Display labels are for people; API operations identify tickets by their opaque ticket ID.</p></section><section><small>CUSTOMER TRACKING</small><h3>Unavailable</h3><p>Mobile account linking is not exposed in this portal yet, so no customer details are shown here.</p><p>Lifecycle updates come from the developer’s software. This inspection view does not call, serve, skip or cancel tickets.</p></section></div></td></tr>}</Fragment>; })}</tbody></table></div>
...
function WebhooksPage({ webhooks, selectedWebhook, selectedWebhookValue, deliveries, busy, onWebhookChange, onCreatePage, onRotate, onCompromised, onDisable, onRefresh, onReplay }: { project: Project; webhooks: Webhook[]; selectedWebhook: string; selectedWebhookValue: Webhook | null; deliveries: Delivery[]; busy: string; onWebhookChange: (value: string) => void; onCreatePage: () => void; onRotate: (webhook: Webhook) => void; onCompromised: (webhook: Webhook) => void; onDisable: (webhook: Webhook) => void; onRefresh: () => void; onReplay: (delivery: Delivery) => void }) {
  ...
  const deliveryRows = deliveries.map(liveDeliveryRow);
  const query = search.trim().toLowerCase();
...
function UsagePage({ project, allowance, usage }: { project: Project; allowance: SandboxAllowance | null; usage: UsageReport | null }) {
  ...
  const recentTickets = usage?.recentTickets || [];
  const ticketQuery = ticketSearch.trim().toLowerCase();
  const filteredRecentTickets = recentTickets.filter((ticket) => {
    const searchable = `${ticket.id} ${ticket.ticketNumber} ${ticket.displayLabel || ""} ${ticket.externalReference || ""} ${ticket.queueId} ${ticket.status}`.toLowerCase();
    return (!ticketQuery || searchable.includes(ticketQuery)) && (!ticketStatusFilter || ticket.status === ticketStatusFilter);
  });
  const ticketPageCount = Math.max(1, Math.ceil(filteredRecentTickets.length / ticketPageSize));
  const currentTicketPage = Math.min(ticketPage, ticketPageCount - 1);
  const ticketPageRows = filteredRecentTickets.slice(currentTicketPage * ticketPageSize, (currentTicketPage + 1) * ticketPageSize);
  const allVisibleTicketsExpanded = ticketPageRows.length > 0 && ticketPageRows.every((ticket) => expandedTicketIds.includes(ticket.id));
  const resetTicketFilters = () => { setTicketPage(0); setExpandedTicketIds([]); };
  const dailyRows = usage?.daily || [];
  const dailyPageCount = Math.max(1, Math.ceil(dailyRows.length / dailyPageSize));
  const currentDailyPage = Math.min(dailyPage, dailyPageCount - 1);
  const dailyPageRows = dailyRows.slice(currentDailyPage * dailyPageSize, (currentDailyPage + 1) * dailyPageSize);
  return <section className="developer-workspace-panel dpp-section developer-workspace-usage-page">
    ...
    <Paper withBorder p="xl" className="developer-workspace-usage-history">
      ...
      {usage && dailyRows.length > 0 && <><div className="dpp-delivery-table-wrap developer-workspace-usage-table-wrap" role="region" aria-label="Daily Sandbox ticket usage" tabIndex={0}><table className="dpp-delivery-table developer-workspace-usage-table"><caption>Daily ticket issuance · Sandbox</caption><thead><tr><th scope="col">Date (UTC)</th><th scope="col">Tickets issued</th></tr></thead><tbody>{dailyPageRows.map((day) => <tr key={day.date}><th scope="row">{day.date}</th><td>{day.issuedTickets}</td></tr>)}</tbody></table></div><div className="dpp-table-pagination developer-workspace-ticket-pagination"><Select label="Rows per page" data={["10", "25", "50"]} value={String(dailyPageSize)} allowDeselect={false} onChange={(value) => { setDailyPageSize(Number(value || 10)); setDailyPage(0); }} comboboxProps={{ withinPortal: false }} classNames={{ dropdown: "dpp-select-dropdown", option: "dpp-select-option" }} /><span role="status">{currentDailyPage * dailyPageSize + 1}–{Math.min((currentDailyPage + 1) * dailyPageSize, dailyRows.length)} of {dailyRows.length} days</span><div><Button type="button" variant="light" disabled={currentDailyPage === 0} onClick={() => setDailyPage(currentDailyPage - 1)}>Previous</Button><Button type="button" variant="light" disabled={currentDailyPage >= dailyPageCount - 1} onClick={() => setDailyPage(currentDailyPage + 1)}>Next</Button></div></div></>}
      {usage && <section className="developer-workspace-usage-ticket-history"><div className="developer-workspace-resource-header"><div><p className="developer-workspace-eyebrow">TICKET HISTORY</p><h2>Recent tickets</h2><p>Expand a row to inspect the ticket identifiers, timestamps, and lifecycle state.</p></div></div><div className="dpp-ticket-filters"><TextInput label="Search" placeholder="Ticket, reference, or queue ID" value={ticketSearch} onChange={(event) => { setTicketSearch(event.currentTarget.value); resetTicketFilters(); }} /><Select label="Status" placeholder="All statuses" data={[...new Set(recentTickets.map((ticket) => ticket.status))]} value={ticketStatusFilter} onChange={(value) => { setTicketStatusFilter(value); resetTicketFilters(); }} comboboxProps={{ withinPortal: false }} classNames={{ dropdown: "dpp-select-dropdown", option: "dpp-select-option" }} /><Select label="Mobile tracking" placeholder="Not connected" data={[]} disabled /></div><div className="dpp-filter-summary"><span role="status">{filteredRecentTickets.length} of {recentTickets.length} tickets</span><div><Button type="button" variant="subtle" disabled={!ticketSearch && !ticketStatusFilter} onClick={() => { setTicketSearch(""); setTicketStatusFilter(null); resetTicketFilters(); }}>Clear filters</Button><Button type="button" variant="light" disabled={!filteredRecentTickets.length} onClick={() => setExpandedTicketIds(allVisibleTicketsExpanded ? [] : ticketPageRows.map((ticket) => ticket.id))}>{allVisibleTicketsExpanded ? "Collapse page" : "Expand page"}</Button></div></div><p className="dpp-table-scroll-hint">On narrow screens, scroll horizontally to view every column.</p><div className="dpp-delivery-table-wrap developer-workspace-usage-table-wrap" role="region" aria-label="Recent Sandbox tickets" tabIndex={0}><table className="dpp-delivery-table developer-workspace-ticket-table"><caption>Recent tickets · Sandbox · ticket history</caption><thead><tr><th scope="col"><span className="dpp-sr-only">Expand ticket</span></th><th scope="col">Ticket</th><th scope="col">Status</th><th scope="col">Issued</th><th scope="col">Mobile tracking</th><th scope="col">Queue ID</th></tr></thead><tbody>{!filteredRecentTickets.length && <tr><td colSpan={6}><h3>{recentTickets.length ? "No matching tickets" : "No tickets yet"}</h3><p>{recentTickets.length ? "Try another search or clear the filters." : "Tickets issued through your integration will appear here."}</p></td></tr>}{ticketPageRows.map((ticket) => { const open = expandedTicketIds.includes(ticket.id); const toggle = () => setExpandedTicketIds(open ? expandedTicketIds.filter((id) => id !== ticket.id) : [...expandedTicketIds, ticket.id]); return <Fragment key={ticket.id}><tr className="dpp-delivery-parent" tabIndex={0} aria-expanded={open} aria-controls={`usage-ticket-${ticket.id}`} aria-label={`${ticket.ticketNumber}, ${ticket.status}`} onClick={toggle} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); toggle(); } }}><td><span className={`dpp-row-chevron${open ? " is-open" : ""}`} aria-hidden="true"><svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="m9 5 7 7-7 7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg></span></td><th scope="row">{ticket.ticketNumber || ticket.displayLabel || ticket.id}</th><td><Badge color={ticket.status === "served" ? "teal" : ticket.status === "cancelled" || ticket.status === "unserved" ? "red" : ticket.status === "called" || ticket.status === "waiting" ? "blue" : "gray"} variant="light" radius="xl">{ticket.status}</Badge></td><td>{formatDate(ticket.createdAt)}</td><td>Unavailable</td><td><code>{ticket.queueId}</code></td></tr>{open && <tr className="dpp-delivery-note"><td colSpan={6}><div className="dpp-webhook-detail" id={`usage-ticket-${ticket.id}`}><section><small>TICKET DETAILS / SANDBOX</small><h3>{ticket.ticketNumber || ticket.displayLabel || ticket.id}</h3><dl><div><dt>Ticket ID</dt><dd><code>{ticket.id}</code></dd></div><div><dt>Display label</dt><dd>{ticket.displayLabel || ticket.ticketNumber || "None"}</dd></div><div><dt>Created at</dt><dd>{formatDate(ticket.createdAt)}</dd></div><div><dt>Updated at</dt><dd>{formatDate(ticket.updatedAt)}</dd></div><div><dt>Status</dt><dd>{ticket.status}</dd></div><div><dt>External reference</dt><dd>{ticket.externalReference || "None"}</dd></div><div><dt>Queue ID</dt><dd><code>{ticket.queueId}</code></dd></div><div><dt>Status reason</dt><dd>{ticket.statusReason || "None"}</dd></div></dl><p>Display labels are for people; API operations identify tickets by their opaque ticket ID.</p></section><section><small>CUSTOMER TRACKING</small><h3>Unavailable</h3><p>Mobile account linking is not exposed in this portal yet, so no customer details are shown here.</p><p>This history is read-only. Lifecycle updates come from your integration’s ticket operations.</p></section></div></td></tr>}</Fragment>; })}</tbody></table></div><div className="dpp-table-pagination developer-workspace-ticket-pagination"><Select label="Rows per page" data={["10", "25", "50"]} value={String(ticketPageSize)} allowDeselect={false} onChange={(value) => { setTicketPageSize(Number(value || 10)); resetTicketFilters(); }} comboboxProps={{ withinPortal: false }} classNames={{ dropdown: "dpp-select-dropdown", option: "dpp-select-option" }} /><span role="status">{filteredRecentTickets.length ? currentTicketPage * ticketPageSize + 1 : 0}–{Math.min((currentTicketPage + 1) * ticketPageSize, filteredRecentTickets.length)} of {filteredRecentTickets.length}</span><div><Button type="button" variant="light" disabled={currentTicketPage === 0} onClick={() => { setTicketPage(currentTicketPage - 1); setExpandedTicketIds([]); }}>Previous</Button><Button type="button" variant="light" disabled={currentTicketPage >= ticketPageCount - 1} onClick={() => { setTicketPage(currentTicketPage + 1); setExpandedTicketIds([]); }}>Next</Button></div></div></section>}
...
export default function DeveloperWorkspace({ session, accountContent, light = false }: Props) {
  ...
  async function createQueue(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const formElement = event.currentTarget; const form = new FormData(formElement);
    const saved = await submit("queue", async () => { await developerApi.createQueue(projectId, selectedProfile, { slug: String(form.get("slug") || ""), displayName: String(form.get("displayName") || ""), sessionState: String(form.get("sessionState") || "closed"), intakeEnabled: form.get("intakeEnabled") === "on", queuePrefix: String(form.get("queuePrefix") || "").trim() || undefined, averageServiceMinutes: Number(form.get("averageServiceMinutes") || 15), notificationThreshold: Number(form.get("notificationThreshold") || 2) }, session.csrfToken); const result = await developerApi.queues(projectId, selectedProfile); setQueues(result.queues); setQueueSnapshots(result.snapshots || []); formElement.reset(); });
    if (saved) navigate("queues", "sandbox");
  }
  async function updateQueue(queue: Queue, event: FormEvent<HTMLFormElement>) {
    const form = new FormData(event.currentTarget);
    return submit(`queue-update-${queue.id}`, async () => { await developerApi.updateQueue(projectId, selectedProfile, queue.slug, { displayName: String(form.get("displayName") || ""), sessionState: String(form.get("sessionState") || queue.sessionState), intakeEnabled: form.get("intakeEnabled") === "on", queuePrefix: String(form.get("queuePrefix") || "").trim(), averageServiceMinutes: Number(form.get("averageServiceMinutes") || queue.averageServiceMinutes), notificationThreshold: Number(form.get("notificationThreshold") || queue.notificationThreshold), resourceVersion: queue.resourceVersion }, session.csrfToken); const result = await developerApi.queues(projectId, selectedProfile); setQueues(result.queues); setQueueSnapshots(result.snapshots || []); });
  }
  ...
  async function createAppleReviewAccount() {
    await submit("test-account-review-create", async () => {
      const result = await developerApi.createAppleReviewAccount(projectId, session.csrfToken);
      setSecret({ label: "Apple App Review Sandbox credentials", value: `Username: ${result.credentials.username}\nEmail: ${result.credentials.email}\nPassword: ${result.credentials.password}`, warning: result.warning });
      await refreshWorkspace();
    });
  }
  ...
  return <section className="developer-workspace dpp dpp-A" data-api-theme={light ? "light" : "dark"}>
    ...
    {loading ? <p className="developer-workspace-loading">Loading your workspace…</p> : isAccountPage ? accountContent : <>
      ...
      {["billing", "subscriptions", "teamSecurity", "security"].includes(section) ? null : !project ? <section className="developer-workspace-panel dpp-section"><p className="developer-workspace-eyebrow">FIRST STEP</p><h1>Create your Sandbox project</h1><p>A project owns its keys, profiles, queues, and webhook registrations. The current plan supports one active project.</p><form onSubmit={(event) => void createProject(event)} className="developer-workspace-form"><TextInput label="Project name" name="name" required maxLength={80} placeholder="Harbor Services integration" /><Button type="submit" className="developer-workspace-primary" loading={busy === "project"} leftSection={<IconPlus size={16} />}>Create project</Button></form></section> : <>
        {section === "setup" && <Setup allowance={allowance} testAccounts={testAccounts} busy={busy} onCreate={() => void createTestAccount()} onCreateAppleReview={() => void createAppleReviewAccount()} onReset={(account) => void resetTestAccount(account)} />}
