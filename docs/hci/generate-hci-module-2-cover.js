...
const out = path.join(__dirname, "getprio-hci-module-2-cover.pdf");
...
const details = {
  ...
  module: "HCI Module 2",
  deliverable: "User Research Report & Personas",
  figmaUrl:
    "https://www.figma.com/design/rwKhBnsKjEBprhQpC5kiez/GetPrio-HCI---IAS--UI-UX-?node-id=57-2"
};
...
function labelValue(label, value, x, y, w = 210) {
  ...
  paragraph(value, x, y - 20, Math.floor(w / 5.5), 14, 17, { bold: true, color: colors.ink });
}

function bullet(value, x, y, maxChars, color) {
  rect(x, y + 5, 5, 5, color);
  return paragraph(value, x + 14, y + 10, maxChars, 9.5, 13, { color: colors.ink });
}

function card(title, bullets, x, y, w, h, fillColor, accentColor) {
  rect(x, y, w, h, fillColor, accentColor);
  text(title, x + 18, y + h - 34, 16, { bold: true, color: accentColor });
  let cursor = y + h - 78;
  for (const item of bullets) {
    cursor = bullet(item, x + 20, cursor, Math.floor((w - 48) / 5.2), accentColor) - 10;
  }
}

rect(0, 0, W, H, colors.canvas);
centered("HCI MODULE 2 DELIVERABLE", 770, 10, { bold: true, color: colors.orange });
centered("GetPrio", 728, 34, { bold: true, color: colors.ink });
centered("User Research Report & Personas", 694, 17, { bold: true, color: colors.ink });
paragraph(
  "Cover sheet for the HCI Module 2 Figma boards and PDF deliverable for a multi-tenant QR-based digital queue management capstone project.",
  86,
  662,
  72,
  10,
...
labelValue("Deliverable", details.deliverable, 336, 474);

rect(54, 332, 487, 84, colors.blueSoft, colors.blue);
text("Figma Board URL", 78, 382, 13, { bold: true, color: colors.blue });
paragraph(details.figmaUrl, 78, 360, 86, 8.4, 11, { color: colors.blue });

card(
  "Included Boards",
  [
    "Research report overview with objectives and assumptions.",
    "Research method and key findings.",
    "Persona overview map for customer, vendor staff, vendor admin, and platform admin.",
    "Detailed persona boards and research-to-design traceability."
  ],
  54,
  128,
  234,
...
card(
  "Security Scope",
  [
    "MVP includes password login, JWT/session handling, RBAC, OTP, and CAPTCHA states.",
    "OAuth2 is documented as a planned post-MVP admin sign-in enhancement."
  ],
  307,
  128,
  234,
