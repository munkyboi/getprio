...
export async function startCheckout(token: string, tenantSlug: string, body: import("@shared").CreateCheckoutRequest) {
  ...
  const checkoutPayload = { ...payload, billingMode: body.billingMode, paymentMethod: body.paymentMethod };
  const preview=await apiRequest<{confirmation:{token:string};preview:{revision:string}}>(`/billing/tenant/${tenantSlug}/commercial-actions/preview`,{method:"POST",token,body:{action:"subscription.checkout",reason,payload:checkoutPayload}});
  return apiRequest<import("@shared").CheckoutSessionResponse, typeof payload & {reason:string;previewRevision:string}>(
    ...
    { method: "POST", token,headers:{"X-Transaction-Confirmation":preview.confirmation.token}, body:{...checkoutPayload,reason,previewRevision:preview.preview.revision} }
  );
}
