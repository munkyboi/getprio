...
function formatCampaignEventType(value = "") {
  return value
    .replaceAll("_", " ")
...
export default function CampaignControlCenterPage() {
  ...
  const fundingTargetCents = campaign
    ...
    : 0;
  const remainingToTargetCents = Math.max(0, fundingTargetCents - acceptedAmountCents);
  ...
  return <><Container className="campaign-control-page" p={0} size="lg"><Stack gap="lg">
    ...
    {isOrganizer && campaign.status === "draft" ? <DraftCampaignEditor campaign={campaign} token={token} onSaved={() => void load()}/> : null}
    {isOrganizer ? <Card className="campaign-funding-summary" p="lg"><Stack gap="md">
      <Group align="flex-start" justify="space-between">
        <div><Text className="finazze-section-label">Campaign summary</Text><Title order={3}>Funding breakdown</Title></div>
        <Badge color={remainingToTargetCents > 0 ? "orange" : "teal"} variant="light">{acceptedContributors}/{campaign.requiredContributors} confirmed</Badge>
      </Group>
      <Paper className="campaign-funding-summary__target" p="md" radius="md" withBorder>
        <Group align="flex-end" justify="space-between">
          <Stack gap={2}><Text c="dimmed" size="sm">Target fund</Text><Text fw={900} size="xl">{money(fundingTargetCents, campaign.currency)}</Text></Stack>
          <Text c="dimmed" size="xs" ta="right">{money(campaign.contributionFeeCents, campaign.currency)} × {campaign.requiredContributors} slots</Text>
        </Group>
      </Paper>
      <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="sm">
        <Paper p="md" radius="md" withBorder><Text c="dimmed" size="sm">Confirmed funds</Text><Text c="teal" fw={800}>{money(acceptedAmountCents, campaign.currency)}</Text></Paper>
        <Paper p="md" radius="md" withBorder><Text c="dimmed" size="sm">Remaining to target</Text><Text c={remainingToTargetCents > 0 ? "orange" : "teal"} fw={800}>{money(remainingToTargetCents, campaign.currency)}</Text></Paper>
        <Paper p="md" radius="md" withBorder><Text c="dimmed" size="sm">Contribution per person</Text><Text fw={800}>{money(campaign.contributionFeeCents, campaign.currency)}</Text></Paper>
        <Paper p="md" radius="md" withBorder><Text c="dimmed" size="sm">Contributor slots</Text><Text fw={800}>{campaign.requiredContributors} people</Text></Paper>
      </SimpleGrid>
      {campaign.status === "collecting" ? <Group className="campaign-summary-actions" justify="flex-end"><Button disabled={busy} onClick={unpublish} variant="light">Unpublish</Button><Button color="red" disabled={busy} onClick={cancel} variant="light">Cancel campaign</Button></Group> : null}
    </Stack></Card> : null}
