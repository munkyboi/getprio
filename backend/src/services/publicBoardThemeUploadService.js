...
function assertB2Configured() {
  ...
  if (missing.length) {
    const error = new Error(`Backblaze B2 uploads are not configured. Missing: ${missing.join(", ")}.`);
...
function buildObjectKey({ tenantId, location, assetType, fileName, contentType }) {
  ...
  return `public-board-themes/tenants/${tenantId}/${locationPart}/${assetType}/${Date.now()}-${randomId}.${extension}`;
}
...
async function createUpload({ tenant, location, user, body }) {
  assertB2Configured();
