  ...
  const { rows } = await db.pool.query(
    `SELECT campaigns.*, bookings.scheduled_start_at,
       ...
       COALESCE((
         SELECT COUNT(*)::int
         FROM organizer_campaign_contributions
         WHERE campaign_id = campaigns.id AND contribution_status <> 'rejected'
...
async function createContribution({ campaignId, contributorUserId, amountCents }) {
  return db.withTransaction(async (client) => {
    const campaignResult = await client.query(
      "SELECT id, required_contributors FROM organizer_campaigns WHERE id = $1 AND campaign_status = 'collecting' AND deadline_at > NOW() FOR UPDATE",
      [Number(campaignId)]
    );
    const campaign = campaignResult.rows[0];
    if (!campaign) return null;
    const countResult = await client.query(
      "SELECT count(*)::int AS count FROM organizer_campaign_contributions WHERE campaign_id = $1 AND contribution_status <> 'rejected'",
      [Number(campaignId)]
    );
    if (Number(countResult.rows[0]?.count || 0) >= Number(campaign.required_contributors)) return null;
    const { rows } = await client.query(
      "INSERT INTO organizer_campaign_contributions (campaign_id, contributor_user_id, amount_cents) VALUES ($1,$2,$3) RETURNING *",
      [Number(campaignId), Number(contributorUserId), amountCents]
    );
...
async function submitContributionProof({ contributionId, paymentReference, proof }) {
  return db.withTransaction(async (client) => {
    ...
    if (current.contribution_status === "rejected") {
      const countResult = await client.query("SELECT count(*)::int AS count FROM organizer_campaign_contributions WHERE campaign_id = $1 AND contribution_status <> 'rejected'", [Number(current.campaign_id)]);
